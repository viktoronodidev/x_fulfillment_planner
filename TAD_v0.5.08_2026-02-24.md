...
**Implemented Baseline:** v0.5.08 (release date: 2026-02-16)  
...
- BOM handling is default BOM only; no multi-BOM decision logic.
...
**Out of scope / roadmap (not implemented in v0.5.08):**
...
| `source_type` | Selection | so, mo, po | Yes | so | - | No | scoped | only `so` used in v0.5.08 |
...
### 15.2 Roadmap reference (NOT implemented in v0.5.08)
- **v0.6:** Purchase Order & PO line shortage from aggregation (demand from batch-created MOs).
- **v0.7:** Multi-BOM logic.
- **v0.8:** Additional polishing + validation revamp.
...
This document is based on repository inspection of the implemented code baseline on branch `0.5.1-dev-20260216` (module version `17.0.0.5.08`), while the file is created on `main` as requested.
